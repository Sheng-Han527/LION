...
wandb.init(
    ...
    project="Latest New Variable Noise",
...
savefolder = pathlib.Path("/local/scratch/public/sh2146/LastestVari")

final_result_fname = savefolder.joinpath("FBPConvNet_vari_noise_final_iter.pt")
checkpoint_fname = savefolder.joinpath("FBPConvNet_vari_noise_check_*.pt")  
validation_fname = savefolder.joinpath("FBPConvNet_vari_noise_min_val.pt")
...
for epoch in range(start_epoch, train_param.epochs):
    ...
    for sinogram, target_reconstruction in tqdm(lidc_validation):
        ...
        # wandb.log({"valid_loss": loss}, step=idx + epoch*len(lidc_validation))
        #add noise to sino
        
        valid_loss += loss.item()
    
    init_psnr=psnr_module(fdk(sinogram,model.op),target_reconstruction,'mean')
    final_psnr=psnr_module(reconstruction,target_reconstruction,'mean')
    wandb.log({"init_psnr": init_psnr, "final_psnr": final_psnr}, step= (epoch+1)*len(lidc_dataloader))
    print(
        f"""Epoch {epoch+1} \t\t Noise parameter:{t} \t\t Training Loss: {train_loss / len(lidc_dataloader)} \t\t Validation Loss: {valid_loss / len(lidc_validation)}\n
         Initial PSNR {init_psnr} Final PSNR {final_psnr}"""
    )
