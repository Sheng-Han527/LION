...
# psnr_module = psnr_metric.PSNR()

# start a new wandb run to track this script
wandb.init(
    # set the wandb project where this run will be logged
    project="Fixed Noise t=0.0",
...
savefolder = pathlib.Path("/local/scratch/public/sh2146/Unet0")

final_result_fname = savefolder.joinpath("FBPConvNet_fixed_noise_t=0_final_iter.pt")
checkpoint_fname = savefolder.joinpath("FBPConvNet_fixed_noise_t=0_check_*.pt")  
validation_fname = savefolder.joinpath("FBPConvNet_fixed_noise_t=0_min_val.pt")
#
# #%% Define experiment
...
def plot_reconstruction( recon, target_recon):
    ...
    plt.clim(0, 2.5)
    plt.colorbar()
    plt.savefig("fixed_modelt=0.0_reconstruction.png")
...
#     for sinogram, target_reconstruction in tqdm(lidc_dataloader):
    ...
    valid_loss = 0.0
    model.eval()
    for i, sinogram, target_reconstruction in enumerate(tqdm(lidc_validation)):
        with torch.no_grad():
            for i in range(sinogram.shape[0]):
                sinogram[i]=ct.sinogram_add_noise(sinogram[i],I0=I0)
        sinogram.to(device)
        ...
        wandb.log({"valid_loss": loss}, step=i + epoch*len(lidc_validation))
        #add noise to sino
